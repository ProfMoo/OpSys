...
int main()
{
  ...
  struct hostent * hp = gethostbyname( "TheOffice" );
  ...
  bufferPut = calloc( 1, lSize+1 );
  if( !bufferPut ) fclose(fp),fputs("memory alloc fails",stderr),exit(1);

  /* copy the file into the bufferPut */
  if( 1!=fread( bufferPut , lSize, 1 , fp) )
    fclose(fp),free(bufferPut),fputs("entire read fails",stderr),exit(1);

  n = write( sd, bufferPut, 342 );
